...
console.log(counter()) // 0   - повертає 1
console.log(counter()) // 1    - повертає 2
 ...
 * - Функція myPrint(a, b, res) - глобальна функція, що має генерувати з параметрів a, b, res 
 * рядок вигляду 'a^b=res' і повертати його.
...
const myPow = (a, b, processResult) => {
    function pw(a, b) {
        if (b==0) {return 1;}
        else if (b==1) {return a;}
        else {return a * pw(a, b - 1);}
    }
    if (b<0) {return processResult(a, b, 1/pw(a, Math.abs(b)));}
    else {return processResult(a, b, pw(a, b));}
}
